...
const validNets = new Set(Object.keys(nets).map(Number));
...
while (i < lines.length) {
  ...
  const rawNet = parseInt(fields[7], 10);
  const schoolNet = validNets.has(rawNet) ? [rawNet] : [];
  ...
  schools.push({
    ...
    campusArea: fields[8].trim(),
    classroomCount: parseInt(fields[9], 10) || 0,
    specialRooms: fields[10].trim(),
    facilities: fields[11].trim(),
  });
}
